...
class Window():
    ...
    def go(self, event):
        ...
        global found_path
        for id in found_path:
            self.canvas.delete(id)
        path = self.star_search(self.heuristic_1, [self.get_start_position()], 60)
        path = self.star_search(self.heuristic_2, path, 40)
        found_path = self.draw_result(path)
    ...
    def heuristic_1(self, position):
        distance, rotation_1, rotation_2 = self.path_parameters(position)
        rotation = rotation_1 + rotation_2
        return 2000 * rotation_1

    def heuristic_2(self, position):
        distance, rotation_1, rotation_2 = self.path_parameters(position)
        rotation = rotation_1 + rotation_2
        return distance + 450 * rotation_1 + 100 * rotation_2
